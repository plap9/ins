...
export const createComment = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (parent_id) {
            const [[parentComment]] = await pool.query<RowDataPacket[]>(
                'SELECT comment_id FROM comments WHERE comment_id = ? AND post_id = ?',
                [parent_id, postId]
            );
            if (!parentComment) return next(new AppError('Bình luận cha không tồn tại', 404));
        }
        ...
        if (result.affectedRows === 0) return next(new AppError('Lỗi khi thêm bình luận', 500));

        await pool.query('UPDATE posts SET comment_count = comment_count + 1 WHERE post_id = ?', [postId]);
...
export const deleteComment = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    ...
    try {
        ...
        if (!comment) return next(new AppError('Bình luận không tồn tại', 404));
        if (comment.user_id !== userId) return next(new AppError('Bạn không có quyền xóa bình luận này', 403));

        await connection.beginTransaction();

        const [childComments] = await connection.query<RowDataPacket[]>(
        ...
        if (childComments.length > 0) {
            ...
            for (const child of childComments) {
                const [grandchildren] = await connection.query<RowDataPacket[]>(
                    'SELECT COUNT(*) as count FROM comments WHERE parent_id = ?',
                    [child.comment_id]
                );
                deleteCount += grandchildren[0].count;
            }
        }

        await connection.query('DELETE FROM comments WHERE comment_id = ? OR parent_id = ?', [commentId, commentId]);
...
export const likeComment = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (liked) return next(new AppError('Bạn đã thích bình luận này rồi', 400));
        ...
        if (result.affectedRows === 0) return next(new AppError('Lỗi khi thích bình luận', 500));
...
export const unlikeComment = async (req: AuthRequest, res: Response, next: NextFunction): Promise<void> => {
    try {
        ...
        if (!liked) return next(new AppError('Bạn chưa thích bình luận này', 400));
        ...
        if (result.affectedRows === 0) return next(new AppError('Lỗi khi bỏ thích bình luận', 500));
